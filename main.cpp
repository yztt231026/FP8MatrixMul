    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < S; ++j) {
            ...
            for (int k = 0; k < L; ++k) {
                ...
                // int offset = 0;
                //  3. 查表并累加
                //  注意：__fp16 会自动隐式转换为 float 进行加法以保证精度
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < S; ++j) {
            ...
            while (svptest_any(svptrue_b32(), pg)) {
                ...
                // 使用 index 版本：它会自动将索引乘以 sizeof(float)
                // 如果编译器还报找不到，请改为 svld1_gather_index(pg, table, indices)
...
void matmul_sve_fp16(const __fp16 *A, const __fp16 *B_T, float *C, int N, int S, int L)
{
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < S; ++j) {
            ...
            // 归约求和并将结果存入 C[i][j]
            // 注意：这里将 fp16 结果转为 float 存储以提高最终结果精度
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < S; ++j) {
            ...
            while (svptest_any(svptrue_b8(), pg)) {
                ...
                // 2. 核心：点积指令
                // 该指令对 va 和 vb 中每 4 个相邻元素进行点积，累加到 acc_v 对应的 32 位通道
